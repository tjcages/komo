...
for (let i = 0; i < args.length; i += 2) {
  if (
    ...
    throw Error(
      "Use --video film.mp4 [--song song.wav] --mix komo-mix.json --out mixed.mp4",
    );
...
const temp = mkdtempSync(join(tmpdir(), "komo-soundtrack-"));
try {
...
console.log(
  `Exported ${options.out}: music + ${mix.effectsEnabled ? mix.effects.length : 0} effects; ${duration.toFixed(3)}s. Picture copied. Peak headroom gain ${headroom.toFixed(3)}.`,
);
