...
## Vaul comparison and Safari chrome

The owner tested the fixed-sheet and no-blur candidate on iPhone and still found dragging unacceptable. The widget has made no `theme-color` metadata or root-background changes since commit `8658d72`, but the site itself continued to emit explicit light and dark `theme-color` tags. Those site tags are now removed so Safari derives its chrome colors from the site's page colors. The mobile backdrop is transparent while retaining outside-tap dismissal, so opening the Drawer no longer darkens the page area behind Safari's bars.

The pointer method alone did not reproduce Vaul's native feel. The actual Vaul 1.1.2 package with Radix Dialog adds about 21,859 gzip bytes before React; the package's remaining all-features budget was 187 bytes. A diagnostic `/drawer-lab/` page now renders actual Vaul in two places: directly on the page and within a fixed Shadow DOM overlay like komo's. Both use 40 comment rows. A Chrome synthetic touch check delivered all 25 pointer moves to both variants without per-move layout. Owner iPhone comparison of the two variants is needed to locate whether the embedded overlay is the problem or whether the widget's gesture/content path is. The lab is site-only and does not enter the `@tjcages/komo` browser bundle.
