...
`,
 ...
 ${section("03 · Mount on the client", `${code("import { initKomo } from './komo.config.js';\ninitKomo();")}<p><code>komo init</code> generates <code>komo.config.js</code> with your setup baked in — import its <code>initKomo</code> helper and call it once, no arguments; connecting in the next step fills in the final settings. The returned controller’s <code>destroy()</code> removes the tool. In React, skip <code>initKomo</code>: the <code>useKomo</code> hook mounts the same tool and handles cleanup, and the two should never be combined. The tabs below pass the same settings inline, using the key printed after connection.</p>${frameworkTabs}`)}
