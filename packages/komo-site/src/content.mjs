...
const frameworkTabs = `<div class="framework-tabs" data-framework-tabs>
 ...
 <div class="framework-panel" id="framework-panel-react" role="tabpanel" aria-labelledby="framework-tab-react">${code("'use client';\nimport { useKomo } from '@tjcages/komo/react';\n\nexport function Parent() {\n  useKomo({ project: 'YOUR_PROJECT_KEY' });\n  return (\n    <main>\n      ...\n    </main>\n  );\n}")}<p>Call <code>useKomo</code> once in your app or layout component. The hook handles cleanup and Strict Mode — never combine it with a manual <code>initKomo()</code> call. Inline options work; memoize callback options with <code>useCallback</code>. Set <code>enabled: false</code> to remove the tool.</p></div>
 <div class="framework-panel" id="framework-panel-astro" role="tabpanel" aria-labelledby="framework-tab-astro" hidden>${code("<script>\n  import { initKomo } from '@tjcages/komo';\n  initKomo({ project: 'YOUR_PROJECT_KEY' });\n</script>")}<p>With Astro view transitions, destroy the previous controller before swapping the document, then initialize again after navigation.</p></div>
</div>`;
export const pages = [
  {
    path: "/",
...
`,
 ...
 ${section("03 · Mount on the client", `<p>Add one mount for your framework. React uses the <code>useKomo</code> hook — it mounts the tool and handles cleanup, so never call <code>initKomo</code> alongside it. Astro and other frameworks call <code>initKomo</code> with inline options. First time? <code>komo init</code> also generates <code>komo.config.js</code> — import its <code>initKomo</code> and call it with no arguments, and connecting fills in the final settings. The tabs below use the options printed after connection.</p>${frameworkTabs}`)}
 ...
 ${section("Keep it on preview builds", `${code("initKomo({\n  project: 'YOUR_PROJECT_KEY',\n  enabled: import.meta.env.DEV ||\n    import.meta.env.PUBLIC_PREVIEW === 'true',\n});")}<p>Use your framework’s public environment flag. React apps pass the same options to <code>useKomo</code> instead of calling <code>initKomo</code>. Hosted komo uses its own API by default. For self-hosting, pass the API URL as <code>endpoint</code>.</p>`)} `,
  },
  {
    path: "/configuration/",
    label: "Configuration",
