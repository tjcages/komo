...
`,
 ...
 ${section("03 · Mount on the client", `${code("import { initKomo } from '@tjcages/komo';\ninitKomo({ project: 'YOUR_PROJECT_KEY' });")}<p>For first-time setup, <code>komo init</code> generates <code>komo.config.js</code> — import its <code>initKomo</code> helper and call it once, and connecting fills in the final settings. The returned controller’s <code>destroy()</code> removes the tool. In React, skip <code>initKomo</code>: the <code>useKomo</code> hook mounts the same tool and handles cleanup, and the two should never be combined. The examples below use the inline options printed after connection.</p>${frameworkTabs}`)}
