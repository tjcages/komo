...
export const pages = [
 ...
 <article class="tool-card"><div class="tool-card-bar"><span>03 / Setup</span><span data-icon="terminal"></span></div><div class="tool-card-body"><h2>Add komo to your site.</h2><p>Run setup, sign in with Google, and mount komo. Your teammates can sign in or just leave their name.</p>${code("import { initKomo } from '@tjcages/komo';\ninitKomo({ project: 'YOUR_PROJECT_KEY' });")}<a class="text-link" href="/install/">Installation guide <span data-icon="arrow"></span></a></div></article>
 <article class="tool-card"><div class="tool-card-bar"><span>04 / Hosting</span><span data-icon="cloud"></span></div><div class="tool-card-body"><h2>A home for your feedback.</h2><p>Start with komo hosting. No database or hosting to configure.</p><div class="feature-example usage-example" data-usage-demo><div class="usage-demo-controls"><span>Hosted starter</span><div class="scene-controls"><button data-usage-play aria-label="Pause usage animation"><span data-icon="pause"></span></button><button data-usage-replay aria-label="Replay usage animation"><span data-icon="replay"></span></button></div></div><div class="product-preview product-account" aria-label="Example starter plan usage"><div class="product-account-heading"><span data-icon="cloud"></span><strong>Starter plan</strong><span>Hosted</span></div><div class="product-usage"><span>Project comments</span><span><span><span data-usage-comments>42</span> / 250</span> <i class="product-ring" aria-hidden="true"></i></span></div><div class="product-usage"><span>Projects</span><span><strong data-usage-projects>1</strong> of 3</span></div><div class="product-slots" aria-hidden="true"><span class="filled"></span><span></span><span></span></div></div></div><div class="self-host-option"><span class="self-host-icon"><span data-icon="code"></span></span><div><strong>Prefer to self-host?</strong><p>Unlimited projects and comments in your own Cloudflare account. Your infrastructure’s limits apply.</p><a class="text-link" href="/hosting/#self-host">Self-hosting setup <span data-icon="arrow"></span></a></div></div></div></article>
 </section>
 <section class="try-section" aria-labelledby="try-heading"><div class="try-heading-row"><h2 id="try-heading">You’re here. Leave a comment.</h2><div class="scene-controls"><button data-try-motion aria-label="Pause agent cursors"><span data-icon="pause"></span></button></div></div><button id="try-komo" class="try-block" data-try-komo aria-label="Leave your mark — add a comment"><span class="try-content"><span class="try-icon"><span data-icon="comment"></span></span><strong>Leave your mark</strong><span class="try-caption">Click to leave a comment</span></span><span class="try-cursors" aria-hidden="true"><span class="try-cursor-track try-cursor-design"><span class="agent-cursor" data-agent="D"><span data-icon="multiplayer"></span><span data-cursor-name>Design agent</span></span></span><span class="try-cursor-track try-cursor-code"><span class="agent-cursor" data-agent="F"><span data-icon="multiplayer"></span><span data-cursor-name>Frontend agent</span></span></span><span class="try-cursor-track try-cursor-qa"><span class="agent-cursor" data-agent="Q"><span data-icon="multiplayer"></span><span data-cursor-name>QA agent</span></span></span></span></button><p>Public demo · Your latest 3 comments stay.</p></section>
 <div class="home-end"><img src="/favicon.svg" width="26" height="26" alt=""><span>A small tool by <a href="https://offbr.co">Off brand <span data-icon="arrow"></span></a></span></div>`,
  },
  {
    path: "/install/",
    label: "Install",
 ...
 ${section("Self-hosted", `${code("npx @tjcages/komo init --self-host \\\n  --google-client-id YOUR_GOOGLE_CLIENT_ID")}<p>The CLI signs into Cloudflare, creates a D1 database, runs migrations, and deploys a Worker. It prompts for your Google client secret through Wrangler.</p><p>Add the printed callback URL in Google Console, then open the owner-claim link and sign in. Keep the local owner key private.</p>${code("npx @tjcages/komo deploy")}<p>Use this to resume or redeploy an existing setup. Your infrastructure follows your Cloudflare account’s limits and billing. komo does not impose the hosted starter quotas.</p>`, "self-host")}
 ${section("What a shared link means", `<p>komo is a link-accessible review space. Approved sites control browser embedding; they do not enforce private repository membership. Put sensitive previews behind your own access controls.</p><p>Reviewer sessions persist for 30 days on the same site. Different preview domains do not automatically share browser storage.</p>`)}
 ${section("When a project fills up", `<p>Existing feedback stays readable. New comments stop at the limit. Contact <a href="mailto:ty@offbr.co">ty@offbr.co</a> to discuss data export or moving to your own service. Automated import/export and paid upgrades are not available yet.</p>`)} `,
  },
  {
    path: "/agent-prompts/",
 ...
 ${section("A review note becomes a task", `${code("Page: /pricing\nTarget: #annual-plan-button\nElement text: Choose plan\nSource: src/components/Pricing.tsx\n\nMaya: Make it clear this is billed annually.\nAlex: Keep the monthly equivalent visible too.")}<p>komo copies context for your agent; it does not run an agent, upload your repository, or make code changes itself.</p>`)} `,
  },
  {
    ...
    body: `<h1>Questions.</h1>${[
      ...
      [
        "Is feedback private?",
        "Anyone with access to an approved preview and its public project key can read the review space. Use an access gateway for private previews. Origin approval is not an authorization system for repository members.",
      ],
      [
      ...
      [
        ...
        "Contact ty@offbr.co. Automated account removal and data export are not available in the current beta.",
      ],
    ]
      .map(
        ([q, a]) =>
 ...
 ${section("Cookies and local storage", `<p>The widget stores a reviewer session in a cookie and local storage, normally for up to 30 days. It also stores preferences such as drawer position and emoji history. Sign-out revokes the current service session. Cookies are used for authentication, not advertising.</p><p>The animated walkthrough is scripted. Comments you leave on this website are public and shared with other visitors. We keep your latest three messages, including replies, and automatically delete older ones. This demo limit applies to your reviewer identity; a new guest identity starts a new history.</p>`)}
 ${section("Retention and requests", `<p>Hosted feedback remains until removed through service administration. Deleting a comment in the widget replaces its text with a deletion marker; associated records may remain and continue counting toward quota. Resolved comments are retained.</p><p>For an account data copy, correction, or deletion request, email <a href="mailto:ty@offbr.co">ty@offbr.co</a> from an address that can help establish ownership. We may need to verify your identity before fulfilling a request.</p><p>Self-hosted data belongs to the operator of that deployment. Contact that operator about its retention and access rules.</p>`)} `,
  },
  {
    path: "/terms/",
    label: "Terms",
 ...
 ${section("Beta availability", `<p>The hosted service is provided as available, without a service-level commitment. Keep independent records of work you need to retain. Automated export and migration tools are not yet available; contact us for data requests.</p><p>Nothing on this page limits rights that cannot be limited under applicable law. Self-hosted deployments remain the responsibility of their operators.</p>`)}
