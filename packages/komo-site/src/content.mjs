...
`,
 ...
 ${section("03 · Mount on the client", `<p>For first-time setup, mount the generated helper. Connecting fills in its final settings.</p>${code("import { initKomo } from './komo.config.js';\ninitKomo();")}<p>In React, use a temporary setup component so the helper is cleaned up on unmount:</p>${code("'use client';\nimport { useEffect } from 'react';\nimport { initKomo } from './komo.config.js';\n\nexport function KomoSetup() {\n  useEffect(() => {\n    const komo = initKomo();\n    return () => komo.destroy();\n  }, []);\n  return null;\n}")}<p>Render <code>&lt;KomoSetup /&gt;</code> once in your app or layout while connecting. After connection, replace it with the React example below, using the public project key printed in your terminal. Do not keep both mounts. Other frameworks can keep the generated helper or use inline options.</p>${frameworkTabs}`)}
