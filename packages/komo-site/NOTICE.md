...
The agent walkthrough is a scripted illustration with pause, replay, and selectable threads. It does not run agents or alter the pictured website. The actual komo widget is installed on the surrounding site and stores shared visitor feedback in the dedicated hosted service. Automatic per-reviewer deletion in the `komo-landing-demo` project is temporarily disabled during testing. Guest identities persist in the browser; a new guest identity starts a separate history.
