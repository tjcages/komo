...
| `sidebar` | `"background" \| "edge"` | `"background"` | `"background"` frames the site and shows the sidebar in the scaled review sheet. `"edge"` shows a floating, draggable sidebar that parks off and peeks from the viewport edge while closed. Users can switch between the two at runtime from the Account panel; that choice is remembered per project. |
